...
export default function Tugasan() {
  ...
  // The self-check is per outlet. An Area Manager with several outlets picks
  // which (home outlet first); the Manager, who has no home outlet, picks
  // from every outlet — the same list the policies let them write to. Admin
  // may fill in too; other head-office roles that reach this screen by URL
  // get it read-only.
  ...
  const outlets = manager ? (crossBranch ? allOutlets : branchesOf(manager)) : [];
  const [pickedBranch, setPickedBranch] = useState<string | null>(null);
  const branchId =
    pickedBranch && outlets.includes(pickedBranch) ? pickedBranch : (outlets[0] ?? null);
  const canFill =
