...
/**
 * The head-office view. Outlets, not people: a manager, general manager or HR
 * sees how each kedai is performing and where the returns are stuck, and never
 * an individual's marking sheet — that stays with the SV/AS and Area Manager
 * who own it. The Area Manager's tugasan self-check is absent for the same
 * reason: head office reads whether it was done, it does not fill it in.
 */
export default function HeadOffice() {
  ...
  const { total, outlets } = hqReport({
    branches,
    users,
    records,
    markOf: (user, weekIdx) => weekMark(user, weekIdx, submitted),
    // Signed out falls back to the narrowest role; the guard below stops there anyway.
    viewerRole: me?.role ?? 'manager',
  });
  ...
  // Reached by URL without signing in. The report is scoped to the account, so
  // there is nothing honest to show until there is one.
  if (!me) {
    return (
      <Screen>
        ...
        <Card className="p-4 mt-4">
          <Text className="font-sans-med text-[13px] text-ink-3">
            Log masuk sebagai Manager, General Manager atau Human Resources untuk
            melihat laporan cawangan.
          </Text>
        </Card>
      </Screen>
    );
  ...
  return (
    <Screen>
      ...
      <Text className="font-sans text-[13px] leading-5 text-ink-4 mt-1.5">
        {MONTHS[MONTHS.length - 1]} · {outlets.length} cawangan aktif
      </Text>
      ...
      <View className="flex-row gap-2.5">
        ...
        {total.stor ? (
          <Figure
            ...
            foot={`${total.stor.marked} penilaian · ${total.stor.people} pekerja`}
          />
        ) : (
          <OutOfRemit />
        )}
